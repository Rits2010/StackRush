...
import { FileExplorer, createDefaultProjectStructure, createChallengeProjectStructure } from '../components/FileExplorer';
...
const SimulationPage = () => {
  ...
  const [showAPITestModal, setShowAPITestModal] = useState<boolean>(false);
  ...
  const safeChallenge: Challenge = {
    ...
    description: 'Loading challenge description...',
    difficulty: 'medium',
    category: 'algorithms',
    tags: ['loading'],
    starterCode: '// Loading starter code...',
    testCases: [],
    hints: [],
    ...
    const loadChallenge = async () => {
      ...
      try {
        ...
        const transformedChallenge: Challenge = {
          ...
          difficulty: challengeData.difficulty.toLowerCase() as 'easy' | 'medium' | 'hard',
          category: challengeData.category,
          tags: challengeData.tags || [],
          starterCode: challengeData.code?.starterCode?.javascript || '// Starter code not available',
          testCases: challengeData.code?.testCases?.map((tc: any, index: number) => ({
            id: tc._id || `test-${index}`,
          ...
          apiEndpoint: challengeData.apiEndpoint,
          method: challengeData.method || 'POST',
          samplePayload: challengeData.samplePayload
        };
        
        setChallenge(transformedChallenge);
        
        // Set initial code from challenge starter code
        const initialCode = challengeData.code?.starterCode?.javascript || transformedChallenge.starterCode || '// Starter code not available';
        setCode(initialCode);
        ...
        // Initialize project files based on challenge type and actual code
        const files = createChallengeProjectStructure(challengeData);
        setProjectFiles(files);
        
        // Set initial file contents with actual challenge code
        const initialContents: Record<string, string> = {};
        const extractFileContents = (nodes: any[]) => {
          nodes.forEach(node => {
            if (node.type === 'file' && node.content) {
              initialContents[node.path] = node.content;
            }
            if (node.children) {
              extractFileContents(node.children);
            }
          });
        };
        ...
        // Select the main file by default (the one with the starter code)
        const mainFile = files.find(f => f.name === 'src')?.children?.find(f => 
          f.name.includes('payment') || f.name.includes('solution') || f.name.includes('App') || f.name.includes('ProductGallery')
        );
        if (mainFile) {
          setSelectedFile(mainFile.path);
          setCode(mainFile.content || initialCode);
        }
        ...
        setCode(safeChallenge.starterCode || '// Starter code not available');
        
        // Set default project structure
        const files = createDefaultProjectStructure('dsa');
        setProjectFiles(files);
      }
    };
    
    loadChallenge();
  }, [id, challengeType]); // Added challengeType to dependencies
  ...
  // Trigger distractions based on challenge scenario
  useEffect(() => {
    if (!challenge?.scenario?.distractions || mode === 'zen') return;
    
    // Set distraction intervals based on challenge scenario
  ...
  }, [challenge, mode, timeRemaining]); // Removed triggerRandomDistraction from dependencies to avoid cycle
  ...
  // Distraction triggering logic based on mode and distractionLevel
  // Removed the duplicate useEffect that was causing conflicts
  // The distraction logic is now handled in the useEffect that depends on challenge scenario
  ...
  const handleTestComplete = useCallback((results: any[], totalScore: number) => {
    setTestResults(results.map(r => ({
      name: r.testId,
      passed: r.passed,
      message: r.error || (r.passed ? 'Test passed' : 'Test failed')
    })));
    ...
    setTimeout(() => {
      navigate(`/portal/challenge-complete/${id}?success=${success}&score=${totalScore}&time=${formattedTime}&tests=${passedTests}/${results.length}`);
    }, 2000);
  }, [timeRemaining, id, navigate]);
  ...
  return (
    <div className={`h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-800 flex flex-col overflow-hidden ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
      {!isFullscreen && (
          ...
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            ...
            <div className="flex items-center space-x-6">
              <div className="text-center">
                <div className="text-slate-900 dark:text-white font-bold text-xl">{challenge?.title || 'Loading Challenge...'}</div>
                <div className="flex items-center justify-center space-x-4 mt-1">
                  ...
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${
                    challenge?.difficulty === 'Easy' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 border-green-200 dark:border-green-700' :
                    challenge?.difficulty === 'Hard' ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 border-red-200 dark:border-red-700' :
                  ...
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 border border-purple-200 dark:border-purple-700">
                    ...
                    {challenge?.category || challengeType.toUpperCase()}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="p-4 bg-white/80 dark:bg-slate-900/80 border-b border-slate-200/30 dark:border-slate-700/30 backdrop-blur-2xl shadow-lg">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-3">
                ...
                {(challengeType === 'bug-fix' || challenge?.category?.toLowerCase() === 'backend') && (
                  <button
                    onClick={() => setShowAPITestModal(true)}
                    className="flex items-center px-4 py-2 rounded-xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl border bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white border-orange-400/20"
                  >
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex flex-1 min-h-0">
              {/* Enhanced IDE Layout */}
              <div className={`${showPreview && challengeType === 'feature' ? 'w-1/2' : 'flex-1'} flex min-h-0`}>
                {/* File Explorer */}
      ...
      <APITestModal
        ...
        challenge={{
          title: challenge?.title || 'API Testing',
          apiEndpoint: challenge?.apiEndpoint || '/api/auth/login',
          method: challenge?.method || 'POST',
          samplePayload: challenge?.samplePayload || {
